...
class BadStatusCodeTest extends Specification {
  ...
  "An unexpected HTTP status code (not 200, 206, 000 or 304) should not throw an exception" >> {
    Seq("403", "500", "200a") foreach { code =>
      "unexpected status code \"%s\" should not throw a SerDeException".format(code) >> {
