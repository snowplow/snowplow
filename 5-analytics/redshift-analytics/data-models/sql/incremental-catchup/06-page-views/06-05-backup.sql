...
-- Backup the current page views pivot table.

DROP TABLE IF EXISTS snowplow_intermediary.page_views_backup;
CREATE TABLE snowplow_intermediary.page_views_backup
  DISTKEY (domain_userid) -- Optimized to join on other snowplow_intermediary.session_X tables
  SORTKEY (domain_userid, domain_sessionidx, first_touch_tstamp) -- Optimized to join on other snowplow_intermediary.session_X tables
AS (
  SELECT
    *
