...
class Interpreter private(
  ...
  val awsAccessKeyId = cliConfig.configYaml.aws.accessKeyId
  val secretAccessKey = cliConfig.configYaml.aws.secretAccessKey
        ...
        case Track(result) =>
          result match {
            ...
            case ExitLog.LoadingFailed(message, _) =>
              val sanitizedMessage = Common.sanitize(message, List(awsAccessKeyId, secretAccessKey))
