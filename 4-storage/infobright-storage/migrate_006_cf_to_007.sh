...
echo "Creating new Infobright table 0.0.7"
cat ./sql/setup_infobright.sql | ${SQL} -u ${USERNAME} --password=${PASSWORD}
echo "... done."

echo "Updating Infobright table from 0.0.6 (CloudFront collector) to 0.0.7..."
cat ./sql/migrate_0.0.6_cf_to_0.0.7.sql | ${SQL} -u ${USERNAME} --password=${PASSWORD}
echo "... done."
