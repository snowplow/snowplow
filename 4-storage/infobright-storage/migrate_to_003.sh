...
echo "Creating new Infobright table 0.0.3"
cat ./sql/setup_infobright.sql | ${SQL} -u ${USERNAME} --password=${PASSWORD}
echo "... done."

echo "Updating Infobright table from 0.0.1/2 to 0.1.3..."
cat ./sql/migrate_0.0.1-2_to_0.0.3.sql | ${SQL} -u ${USERNAME} --password=${PASSWORD}
echo "... done."
