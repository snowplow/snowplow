...
echo "Creating new Infobright table 0.0.9"
cat ./sql/setup_infobright.sql | ${SQL} -u ${USERNAME} --password=${PASSWORD}
echo "... done."

echo "Updating Infobright table from 0.0.8 to 0.0.9..."
cat ./sql/migrate_0.0.8_to_0.0.9.sql | ${SQL} -u ${USERNAME} --password=${PASSWORD}
echo "... done."
