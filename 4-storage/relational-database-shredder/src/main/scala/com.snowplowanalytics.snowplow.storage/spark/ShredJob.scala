...
class ShredJob(@transient val spark: SparkSession, args: Array[String]) extends Serializable {
  ...
  val dupStorageConfig = DuplicateStorage.DynamoDbConfig.extract(
  ...
  lazy val duplicateStorage: Option[DuplicateStorage] =
    dupStorageConfig.map(DuplicateStorage.initStorage) match {
  ...
  def dedupeCrossBatch(components: (String, String, String)): ValidatedNelMessage[Boolean] =
  ...
  def run(): Unit = {
    ...
    val good = common
      ...
      .map(s => (s, dedupeCrossBatch((s.eventId, s.eventFingerprint, s.etlTstamp))))
      .filter {
        case (_, Success(r)) => r
