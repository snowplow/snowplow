...
object JobSpecHelpers {
   ...
   */
  val ShredJobSpec =
    JobTest("com.snowplowanalytics.snowplow.storage.hadoop.ShredJob").
  ...
  def runJobInTool(lines: Lines, enableCrossbatchDedupe: Boolean = false): Sinks = {
    ...
    val duplicateStorageArgs = if (enableCrossbatchDedupe && SpecHelpers.CI) Array("--duplicate_storage_config", DuplicateStorageConfig) else Array.empty
    val args = Array[String]("com.snowplowanalytics.snowplow.storage.hadoop.ShredJob", "--local",
      ...
      "--iglu_config",       IgluConfig) ++ duplicateStorageArgs
