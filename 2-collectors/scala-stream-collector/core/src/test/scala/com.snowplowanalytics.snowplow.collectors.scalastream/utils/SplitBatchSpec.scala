...
class SplitBatchSpec extends Specification {
  ...
  "SplitBatch.splitAndSerializePayload" should {
    ...
    "Reject an oversized GET CollectorPayload" in {
      ...
      resSdd.schema must_== oversizedBadRow
      val resBr = resSdd.data.as[BadRow].toOption.get
      val f = resBr.failure.asInstanceOf[SizeViolation]
      f.maximumAllowedSizeBytes must_== 100
      f.actualSizeBytes must_== 1019
      f.expectation must_== "oversized collector payload: GET requests cannot be split"
      val p = resBr.payload.asInstanceOf[RawPayload]
      p.line must_== "CollectorP"
      resBr.processor shouldEqual Processor(generated.BuildInfo.name, generated.BuildInfo.version)
    ...
    "Reject an oversized POST CollectorPayload with an unparseable body" in {
      ...
      f.maximumAllowedSizeBytes must_== 100
      f.actualSizeBytes must_== 1019
      f.expectation must_== "oversized collector payload: cannot split POST requests which are not json expected json value got 'ssssss...' (line 1, column 1)"
      val p = resBr.payload.asInstanceOf[RawPayload]
      p.line must_== "CollectorP"
      resBr.processor shouldEqual Processor(generated.BuildInfo.name, generated.BuildInfo.version)
    }
    ...
    "Reject an oversized POST CollectorPayload which would be oversized even without its body" in {
      ...
      val resBr = resSdd.data.as[BadRow].toOption.get
      val f = resBr.failure.asInstanceOf[SizeViolation]
      f.maximumAllowedSizeBytes must_== 1000
      f.actualSizeBytes must_== 1091
      f.expectation must_== "oversized collector payload: cannot split POST requests which are not self-describing INVALID_IGLUURI"
      val p = resBr.payload.asInstanceOf[RawPayload]
      p.line must_== "CollectorPayload(schema:null, ipAddress:null, timestamp:0, encoding:null, collector:null, path:ppppp"
      resBr.processor shouldEqual Processor(generated.BuildInfo.name, generated.BuildInfo.version)
    }
    ...
    "Split a CollectorPayload with three large events and four very large events" in {
      val payload = new CollectorPayload()
      val data = Json.obj(
        "schema" := oversizedBadRow.toSchemaUri,
        "data" := Json.arr(
