...
bintray_artifact_prefix=snowplow_emr_
...
declare -a emr_app_paths=( "3-enrich/emr-etl-runner" "4-storage/storage-loader" )
# TODO: version numbers shouldn't be hard-coded
declare -a emr_fatjars=( "3-enrich/emr-etl-runner/deploy/snowplow-emr-etl-runner" "4-storage/storage-loader/deploy/snowplow-storage-loader" )
...
function warble_fatjars() {
	for emr_app_path in "${emr_app_paths[@]}"
		do
			:
			app="${emr_app_path##*/}"
			echo "================================================"
			echo "WARBLING FATJAR FOR ${app}"
			echo "------------------------------------------------"
			vagrant ssh -c "cd ${guest_repo_path}/${emr_app_path} && ./build.sh"
		done
}
...
function build_artifact() {
	...
	for i in "${!emr_fatjars[@]}"
		do 
			:
			fatjar_path="./${emr_fatjars[$i]}"
