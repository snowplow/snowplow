...
class CljTomcatLoaderSpec
      ...
      (_, raw, version, payload, contentType, body, timestamp, userAgent, refererUri) =>
        {
          ...
          val expected = CollectorPayload(
            ...
            context =
              CollectorContext(timestamp.some, Expected.ipAddress, userAgent, refererUri, Nil, None)
          )
  ...
  def e2 = {
    ...
    actual must beInvalid(
      NonEmptyList.one(
        "Operation must be POST, not GET, if request content type and/or body are provided"))
  }
