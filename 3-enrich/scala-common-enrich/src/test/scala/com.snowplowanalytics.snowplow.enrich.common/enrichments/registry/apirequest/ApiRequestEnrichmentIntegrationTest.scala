...
class ApiRequestEnrichmentIntegrationTest extends Specification with ValidationMatchers {
  ...
  def e2 = {
    ...
    // Fill cache
    config.flatMap(
      _.lookup(event, List(IntegrationTests.weatherContext), List(IntegrationTests.customContexts), List(IntegrationTests.unstructEvent)))
    config.flatMap(
      _.lookup(event, List(IntegrationTests.weatherContext), List(IntegrationTests.customContexts), List(IntegrationTests.unstructEvent)))
    ...
    context must beSuccessful.like {
      case context =>
        context must contain(beJson(IntegrationTests.correctResultContext2), beJson(IntegrationTests.correctResultContext3)) and (context must have size (2))
    } and {
