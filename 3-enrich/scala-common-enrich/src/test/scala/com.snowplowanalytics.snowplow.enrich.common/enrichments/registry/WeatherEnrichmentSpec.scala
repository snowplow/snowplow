...
class WeatherEnrichmentSpec extends Specification  { def is =
  ...
  "Return successful None (skip weather) for null time"          ! e1^
  "Return successful None (skip weather) for invalid key"        ! e5^
  "Weather enrichment client is lazy"                            ! e2^
  "Fail with invalid cache size"                                 ! e3^
  "Extract weather stamp"                                        ! e4^
  "Extract humidity"                                             ! e6^
    ...
    stamp.toEither must beRight.like {                            // successful request
      case Some(weather: JValue) => {
        ...
        e.map(_.dt) must beSome.like {                            // succesfull transformation
          case dt => dt must startingWith("2015-08-29 23:16:14")  // closest stamp storing on server
        }
      }
    }
  }
