...
class CurrencyConversionEnrichmentSpec extends Specification with DataTables { def is =
  ...
  val errorcoTstamp: DateTime = null
  val allAbsent = Failure(NonEmptyList("Exception Converting Currency :java.lang.NumberFormatException: Infinite or NaN"))

  def e1 =
    "SPEC NAME"                         || "TRANSACTION CURRENCY"         | "API KEY"                             | "TOTAL AMOUNTS"                    |"TOTAL TAX"                      |"SHIPPING"                           | "TRANSACTION ITEM CURRENCY"           | "TRANSACTION ITEM PRICE"              | "DATETIME"              |"CONVERTED TUPLE"                               |
    ...
    "Invalid Transaction Currency"      !! None                           ! appId                                 ! Some(12.00)                        ! Some(0.7)                       ! Some(0.00)                          ! Some("HUL")                           ! Some(1.99)                            ! Some(coTstamp)          ! currencyInvalidHul                             |
    "Invalid APP ID"                    !! None                           ! "8A8A8A8A8A8A8A8A8A8A8A8AA8A8A8A8"    ! Some(13.00)                        ! Some(3.67)                      ! Some(0.00)                          ! Some("GBP")                           ! Some(2.99)                            ! Some(coTstamp)          ! appIdInvalid                                   |> {
  ...
  def e2=
    "SPEC NAME"                                  || "TRANSACTION CURRENCY"   | "API KEY"                             | "TOTAL AMOUNTS"  |"TOTAL TAX"  |"SHIPPING"   | "TRANSACTION ITEM CURRENCY"     | "TRANSACTION ITEM PRICE"       | "DATETIME"         |"CONVERTED TUPLE"                                           |
    "All Field Absent"                           !! None                     ! appId                                 ! None             ! None        ! None        ! None                            ! None                           ! None               ! Failure(NonEmptyList("DateTime Missing"))                  |
    "All Field Absent except Currency"           !! Some("GBP")              ! appId                                 ! None             ! None        ! None        ! Some("GBP")                     ! None                           ! None               ! Failure(NonEmptyList("DateTime Missing"))                  |
