...
class CurrencyConversionEnrichmentSpec extends Specification with DataTables { def is =
  ...
  val invalidAppKeyFailure = Failure(NonEmptyList("Open Exchange Rates error, type: [InvalidvalidAppKey], message: [Invalid App ID provided - please sign up at https://openexchangerates.org/signup, or contact support@openexchangerates.org. Thanks!]"))
