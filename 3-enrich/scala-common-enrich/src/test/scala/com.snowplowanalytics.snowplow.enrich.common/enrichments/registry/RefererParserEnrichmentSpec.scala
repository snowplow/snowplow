...
class ExtractRefererDetailsSpec extends Specification with DataTables {
  ...
  def e1 =
    "SPEC NAME"          || "REFERER URI" | "REFERER MEDIUM" | "REFERER SOURCE" | "REFERER TERM" |
      ...
      (_, refererUri, medium, source, term) =>
        RefererParserEnrichment(List("www.internaldomain.com")).extractRefererDetails(new URI(refererUri), PageHost) must_== Some(
  ...
  def e2 =
    RefererParserEnrichment(List()).extractRefererDetails(
      ...
      PageHost) must_== Some(Referer(Medium.Search, Some("Google"), Some("gateway    oracle    cards    denise    linn")))
}
