...
class InputSpec extends Specification with ValidationMatchers { def is =
  ...
  object ContextCase {
    ...
    val unstructInput = Input("unstructValue", pojo = None, json = JsonInput("unstruct", "iglu:com.snowplowanalytics.monitoring.batch/jobflow_step_status/jsonschema/1-0-0", "$.state").some)
