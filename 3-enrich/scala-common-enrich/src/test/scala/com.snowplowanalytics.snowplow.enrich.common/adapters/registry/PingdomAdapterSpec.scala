...
class PingdomAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
  ...
  def e3 = {
    val jsonStr = """{"event":incident_assign"}"""
    val expected =
      "Pingdom event failed to parse into JSON: [com.fasterxml.jackson.core.JsonParseException: Unrecognized token 'incident_assign': was expecting ('true', 'false' or 'null') at [Source: {\"event\":incident_assign\"}; line: 1, column: 25]]"
