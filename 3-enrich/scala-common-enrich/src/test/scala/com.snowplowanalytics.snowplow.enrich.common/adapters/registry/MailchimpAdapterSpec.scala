...
class MailchimpAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck { def is =
  ...
  def e1 = {
    ...
    val payload = CollectorPayload(Shared.api, toNameValuePairs("tv" -> "0", "nuid" -> "123"), ContentType.some, body.some, Shared.cfSource, Shared.context)
    val actual = MailchimpAdapter.toRawEvents(payload)
    actual must beSuccessful(NonEmptyList(RawEvent(Shared.api, Map("tv" -> "ios-0.1.0", "p" -> "mob", "e" -> "se"), None, Shared.cfSource, Shared.context)))
  }
}
