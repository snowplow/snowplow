...
class UnbounceAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
  ...
  def e3 = {
    ...
    UnbounceAdapter.toRawEvents(payload) must beFailing(
      NonEmptyList("Request body provided but content type empty, expected application/x-www-form-urlencoded for Unbounce"))
  }
  ...
  def e4 = {
    ...
    UnbounceAdapter.toRawEvents(payload) must beFailing(
      NonEmptyList("Content type of application/json provided, expected application/x-www-form-urlencoded for Unbounce"))
  }
