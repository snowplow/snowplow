...
class UnbounceAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck { def is =
  ...
  def e8 = {
    ...
    val payload = CollectorPayload(Shared.api, params, ContentType.some, body.some, Shared.cljSource, Shared.context)
    val expected = NonEmptyList("Unbounce event string failed to parse into JSON: [Unexpected character ('{' (code 123)): was expecting double-quote to start field name at [Source: java.io.StringReader@xxxxxx; line: 1, column: 3]]")
    UnbounceAdapter.toRawEvents(payload) must beFailing(expected)
  }
