...
[
  ...
  "toRawEvents" should {
    ...
    "reject unexpected content type" in {
      val invalidpayload =
        CollectorPayload(Shared.api, Nil, "invalidtype/invalid".some, samplePostPayload.some, Shared.cljSource, Shared.context)
      SendgridAdapter.toRawEvents(invalidpayload) must beFailing
    }

    "accept content types with explicit charsets" in {
      val payload =
        CollectorPayload(Shared.api, Nil, "application/json; charset=utf-8".some, samplePostPayload.some, Shared.cljSource, Shared.context)
    ...
    "reject invalid/unparsable json" in {
      ...
      SendgridAdapter.toRawEvents(
        CollectorPayload(Shared.api, Nil, ContentType.some, unparsableJson.some, Shared.cljSource, Shared.context)) must beFailing
    }

    "reject valid json in incorrect format" in {
      val incorrectlyFormattedJson = """[ ]"""
      SendgridAdapter.toRawEvents(
        CollectorPayload(Shared.api, Nil, ContentType.some, incorrectlyFormattedJson.some, Shared.cljSource, Shared.context)) must beFailing
    }
