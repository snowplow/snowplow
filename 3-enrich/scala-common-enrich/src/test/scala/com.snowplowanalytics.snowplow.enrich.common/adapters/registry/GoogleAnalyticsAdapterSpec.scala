...
class GoogleAnalyticsAdapterSpec
  extends Specification with DataTables with ValidationMatchers with DisjunctionMatchers {
  ...
  def e1 = {
    ...
    actual must beFailing(NonEmptyList(
      "Request body is empty: no GoogleAnalytics events to process"))
  }
  ...
  def e2 = {
    ...
    val actual = toRawEvents(payload)
    actual must beFailing(NonEmptyList(
      "No GoogleAnalytics t parameter provided: cannot determine hit type"))
  }
  ...
  def e20 = {
    val s = Seq(
      breakDownCompField("pr") must beRightDisjunction((List("pr"), List.empty[String])),
      breakDownCompField("pr12id") must beRightDisjunction((List("pr", "id"), List("12"))),
      breakDownCompField("12") must beLeftDisjunction("Malformed composite field name: 12"),
      breakDownCompField("") must beLeftDisjunction("Malformed composite field name: "),
      ...
      breakDownCompField("pr", "value", "IF") must beRightDisjunction(Map("pr" -> "value")),
      breakDownCompField("pr", "", "IF") must beRightDisjunction(Map("pr" -> "")),
