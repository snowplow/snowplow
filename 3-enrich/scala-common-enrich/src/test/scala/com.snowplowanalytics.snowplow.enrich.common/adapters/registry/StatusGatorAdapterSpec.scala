...
class StatusGatorAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
  ...
  def e3 = {
    ...
    StatusGatorAdapter.toRawEvents(payload) must beFailing(
      NonEmptyList("Request body provided but content type empty, expected application/x-www-form-urlencoded for StatusGator"))
  }
  ...
  def e4 = {
    ...
    StatusGatorAdapter.toRawEvents(payload) must beFailing(
      NonEmptyList("Content type of application/json provided, expected application/x-www-form-urlencoded for StatusGator"))
  }
