...
class StatusGatorAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck { def is =
  ...
  def e1 = {
    val body = "service_name=CloudFlare&favicon_url=https%3A%2F%2Fdwxjd9cd6rwno.cloudfront.net%2Ffavicons%2Fcloudflare.ico&status_page_url=https%3A%2F%2Fwww.cloudflarestatus.com%2F&home_page_url=http%3A%2F%2Fwww.cloudflare.com&current_status=up&last_status=warn&occurred_at=2016-05-19T09%3A26%3A31%2B00%3A00"
      ...
      """|{
          ...
          |"data":{
            ...
            |"data":{
              ...
              |"favicon_url":"https://dwxjd9cd6rwno.cloudfront.net/favicons/cloudflare.ico",
              |"occurred_at":"2016-05-19T09:26:31+00:00",
    ...
    val expected = NonEmptyList(RawEvent(Shared.api,Map("tv" -> "com.statusgator-v1", "e" -> "ue", "p" -> "srv", "ue_pr" -> expectedJson), ContentType.some, Shared.cljSource, Shared.context))
  ...
  def e2 = {
    ...
    StatusGatorAdapter.toRawEvents(payload) must beFailing(NonEmptyList("Request body is empty: no StatusGator events to process"))
  }

  def e3 = {
    val body = "service_name=CloudFlare&favicon_url=https%3A%2F%2Fdwxjd9cd6rwno.cloudfront.net%2Ffavicons%2Fcloudflare.ico&status_page_url=https%3A%2F%2Fwww.cloudflarestatus.com%2F&home_page_url=http%3A%2F%2Fwww.cloudflare.com&current_status=up&last_status=warn&occurred_at=2016-05-19T09%3A26%3A31%2B00%3A00"
    val payload = CollectorPayload(Shared.api, Nil, None, body.some, Shared.cljSource, Shared.context)
    StatusGatorAdapter.toRawEvents(payload) must beFailing(NonEmptyList("Request body provided but content type empty, expected application/x-www-form-urlencoded for StatusGator"))
  }
  ...
  def e4 = {
    ...
    StatusGatorAdapter.toRawEvents(payload) must beFailing(NonEmptyList("Content type of application/json provided, expected application/x-www-form-urlencoded for StatusGator"))
  }
