...
class IgluAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck { def is =
  ...
  def e15 = {
    val params = toNameValuePairs("schema" -> "iglu:com.acme/campaign/jsonschema/1-0-1")
    val queryStr = "some_param=foo&hello=world"
    val payload = CollectorPayload(Shared.api, params, "application/x-www-form-urlencoded".some, queryStr.some, Shared.cljSource, Shared.context)
    val actual = IgluAdapter.toRawEvents(payload)
