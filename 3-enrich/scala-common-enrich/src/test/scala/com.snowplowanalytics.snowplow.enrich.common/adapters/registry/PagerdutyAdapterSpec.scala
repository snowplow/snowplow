...
class PagerdutyAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck { def is = s2"""
  ...
  def e5 =
    "SPEC NAME"                     || "INPUT"                   | "EXPECTED OUTPUT"                                              |
    "Failure, parse exception"      !! """{"something:"some"}""" ! "PagerDuty payload failed to parse into JSON: [com.fasterxml.jackson.core.JsonParseException: Unexpected character ('s' (code 115)): was expecting a colon to separate field name and value at [Source: {\"something:\"some\"}; line: 1, column: 15]]"     |
