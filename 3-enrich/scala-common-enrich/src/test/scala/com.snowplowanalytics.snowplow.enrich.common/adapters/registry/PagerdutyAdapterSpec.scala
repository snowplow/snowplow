...
class PagerdutyAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck { def is =
  ...
  "payloadBodyToEventList must return a Success list of event JSON's from the PagerDuty payload body"             ! e2^
  "toRawEvents must return a Success containing one RawEvent"                                                     ! e3^
                                                                                                                   end
