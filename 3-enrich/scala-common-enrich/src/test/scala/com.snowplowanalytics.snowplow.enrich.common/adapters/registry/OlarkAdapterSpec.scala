...
class OlarkAdapterSpec extends Specification with DataTables with ValidationMatchers with ScalaCheck {
  ...
  def e4 = {
    ...
    OlarkAdapter.toRawEvents(payload) must beFailing(
      NonEmptyList(
  ...
  def e5 = {
    ...
    OlarkAdapter.toRawEvents(payload) must beFailing(
      NonEmptyList("Content type of application/json provided, expected application/x-www-form-urlencoded for Olark"))
  }
