...
object OlarkAdapter extends Adapter {
          ...
          case TS(bodyMap) => payloadBodyToEvent(bodyMap).flatMap {
            case event  => {
              ...
              lookupSchema(eventType, VendorName, EventSchemaMap).flatMap {
                case schema => transformTimestamps(event).flatMap {
                  case transformedEvent => NonEmptyList(RawEvent(
                    api          = payload.api,
  ...
  private def transformTimestamps(json: JValue): Validated[JValue] = {
    ...
    Try {
      ...
      case TF(e) => s"${VendorName} could not convert timestamps: [${JU.stripInstanceEtc(e.getMessage).orNull}]".failureNel
      case TS(s) => s
    }
  }
    ...
    bodyMap.get("data") match {
      ...
      case Some(json) => {
        try {
          ...
          case NonFatal(e) => s"${VendorName} incorrect event string : [${JU.stripInstanceEtc(e.getMessage).orNull}]".failureNel
        }
      }
    }
  }
