...
object GoogleAnalyticsAdapter extends Adapter {
  ...
  private val unstructEventData: Map[String, MPData] = Map(
    ...
    "screenview" -> MPData(SchemaKey(Vendor, "screen_view", Format, SchemaVersion), Map("cd" -> idTranslation("screenName"))),
    "event" -> MPData(
  ...
  private val contextData: Map[SchemaKey, Map[String, KVTranslation]] = {
    ...
    val ct = unstructEventData(PageViewHitType) :: List(
      MPData(SchemaKey(GaVendor, "undocumented", Format, SchemaVersion), List("a", "jid", "gjid").map(e => e -> idTranslation(e)).toMap),
      MPData(
      ...
      MPData(SchemaKey(Vendor, "user", Format, SchemaVersion), Map("cid" -> idTranslation("clientId"), "uid" -> idTranslation("userId"))),
      MPData(
      ...
      MPData(SchemaKey(Vendor, "promotion_action", Format, SchemaVersion), Map("promoa" -> idTranslation("promotionAction")))
    )
    ...
    params.get("t") match {
      ...
      case Some(hitType) =>
        ...
        val compositeContexts =
          buildCompositeContexts(params, compositeContextData, nrCompFieldsPerSchema, valueInFieldNameIndicator).validation.toValidationNel

        (translationTable |@| schemaVal |@| simpleContexts |@| compositeContexts) { (trTable, schema, contexts, compContexts) =>
   ...
   */
  private[registry] def breakDownCompField(fieldName: String): \/[String, (List[String], List[String])] = fieldName match {
