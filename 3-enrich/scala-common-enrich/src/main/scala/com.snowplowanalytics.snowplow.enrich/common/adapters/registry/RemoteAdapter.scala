  ...
  override def toRawEvents[F[_]: Monad: RegistryLookup: Clock: HttpClient](
    ...
    payload.body match {
      ...
      case _ =>
        ...
        Monad[F].pure(InputDataAdapterFailure("body", none, msg).invalidNel)
    }

  def processResponse(
    payload: CollectorPayload,
    response: Either[Throwable, String]
  ): Either[AdapterFailure, NonEmptyList[RawEvent]] =
    for {
      res <- response
        .leftMap(
          t =>
      ...
      json <- JsonUtils.extractJson(res).leftMap(e => NotJsonAdapterFailure("body", res.some, e))
      events <- json.hcursor
        .downField("events")
      ...
      nonEmptyEvents <- events match {
        case Nil => InputDataAdapterFailure("body", res.some, "empty list of events").asLeft
