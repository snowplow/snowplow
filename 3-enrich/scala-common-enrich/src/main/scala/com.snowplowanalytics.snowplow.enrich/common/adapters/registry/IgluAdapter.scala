...
object IgluAdapter extends Adapter {
      ...
      case Success(_)       => (payload.body, payload.contentType) match {
        ...
        case (Some(body), Some(contentType)) => {
          contentType match {
            ...
            case "application/x-www-form-urlencoded" => qsBodyToEvent(payload, body, schemaUri, params)
            case _ => "Content type not supported".failNel
          }
        }
  ...
  /**
   * Converts a querystring payload into a single validated event
   *
   * @param body querystring payload as POST'd by a webhook
   * @param payload the rest of the payload details
   * @param schemaUri the schemaUri for the event
   * @param params The query string parameters
   * @return a single validated event
   */
  private[registry] def qsBodyToEvent(payload: CollectorPayload, body: String, schemaUri: String, params: Map[String, String]): ValidatedRawEvents = 
    try {
