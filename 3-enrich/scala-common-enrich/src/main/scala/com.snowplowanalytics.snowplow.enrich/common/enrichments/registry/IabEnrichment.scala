...
object IabEnrichment extends ParseableEnrichment {
  ...
  private def getDatabaseUri(uri: String, database: String): ValidatedMessage[URI] = {
    val databaseUri = try {
      new URI(uri).resolve("/" + database).success
    } catch {
      case NonFatal(e) => "URI to IAB file is invalid".failure
    }
    databaseUri.toProcessingMessage
  }
}
...
case class IabEnrichment(
  ...
  private val schemaUri        = "iglu:com.iab.snowplow/iab_spiders_and_robots/jsonschema/1-0-0"
