...
object HeaderExtractorEnrichmentConfig extends ParseableEnrichment {
  val supportedSchema = SchemaCriterion("com.snowplowanalytics.snowplow", "header_extractor_config", "jsonschema", 1, 0)

  /**
   * Creates a HeaderExtractorEnrichment instance from a JValue.
   *
   ...
   */
  def parse(config: JValue, schemaKey: SchemaKey): ValidatedNelMessage[HeaderExtractorEnrichment] = {
    isParseable(config, schemaKey).flatMap(conf => {
      (for {
        headersPattern <- ScalazJson4sUtils.extract[String](config, "parameters", "headersPattern")
        enrich = HeaderExtractorEnrichment(headersPattern)
 ...
 */
case class HeaderExtractorEnrichment(
  ...
  def extract(headers: List[String]): List[JsonAST.JObject] = {
    val http_headers = headers.flatMap { header =>
