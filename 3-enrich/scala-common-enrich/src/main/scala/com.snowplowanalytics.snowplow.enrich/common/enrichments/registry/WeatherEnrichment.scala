...
case class WeatherEnrichment(apiKey: String, cacheSize: Int, geoPrecision: Int, apiHost: String, timeout: Int) extends Enrichment {
   ...
   * It will not interrupt whole enrichment process on any non-fatal errors,
   * like NullPointerException, but return left disjunction,
   * so event will be filtered out in future
   *
  ...
  def getWeatherContext(latitude: Option[JFloat], longitude: Option[JFloat], time: Option[DateTime]): Validation[String, Option[JObject]] =
    if (latitude == null || longitude == null || time == null) { None.success }
    else {
      try {
        getWeather(latitude, longitude, time).map(addSchema).success
      } catch {
   ...
   * It returns None if any of latitude, longitude, time was not presented in event, thus
   * event will not be enriched with weather, but also it will not skipped
   *
   ...
   */
  private def getWeather(latitude: Option[JFloat], longitude: Option[JFloat], time: Option[DateTime]): Option[JObject] =
  ...
  /**
   * Return some weather in case of successful server response or cache hit
   * or none in case of any non-fatal error
   *
   ...
   */
  private def getCachedOrRequest(latitude: Float, longitude: Float, timestamp: Int): Option[Weather] =
    client.getCachedOrRequest(latitude, longitude, timestamp).toOption
  ...
  private[enrichments] def transformWeather(origin: Weather): TransformedWeather = {
    val dateTime = new DateTime(origin.dt.toLong * 1000, DateTimeZone.UTC)
    val dbTime = EventEnrichments.toTimestamp(dateTime)
    TransformedWeather(origin.main, origin.wind, origin.clouds, origin.rain, origin.snow, origin.weather, dbTime)
  }
}
