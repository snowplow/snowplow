    ...
    json match {
      case Some(jsonInput) => {
        val validatedJson = jsonInput.field match {
          ...
          case "unstruct" => getBySchemaCriterion(unstruct.toList, jsonInput.schemaCriterion).successNel
          case other => s"Error: wrong field [$other] passed to Input.getFromJson. Should be one of: derived_contexts, contexts, unstruct".failureNel
        }

        val validatedJsonPath = compileQuery(jsonInput.jsonPath)

        (validatedJsonPath |@| validatedJson) { (jsonPath, validJson) =>
          validJson
...
object Input {
   ...
   * Failure means failure while accessing particular field, like invalid JSON Path, POJO-access, etc
