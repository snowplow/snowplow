  ...
  */
  def extractUserAgent(useragent: String): Validation[String, JsonAST.JObject] = {
    ...
    try {
            ...
            val c = uaParser.parse(useragent)
            
            // To display useragent version
            val useragentVersion = c.userAgent.family + prependSpace(c.userAgent.major) + prependDot(c.userAgent.minor) + prependDot(c.userAgent.patch)
            
            // To display operating system version
            val osVersion = c.os.family + prependSpace(c.os.major) + prependDot(c.os.minor) + prependDot(c.os.patch) + prependDot(c.os.patchMinor)
