...
class JsonAsStringTest extends Specification with DataTables with ValidationMatchers with ScalaCheck { def is =
  ...
  // def e1 = broken by "" -> null due to makeTsvSafe
  //  check { (str: String) => JsonUtils.asString(FieldName, str.asJson) must beSuccessful(str) }
...
class JsonAsJByteTest extends Specification with DataTables with ValidationMatchers with ScalaCheck { def is =
