...
module Snowplow
  module EmrEtlRunner
    module S3Tasks
      ...
      #
      # Removes the log files generated by a previous EMR execution.
      # Prevents storage-loader from loading the same files multiple times.
      #
      # Parameters:
      # +config+:: the hash of configuration options
      #
      def self.delete_previous_execution_logs(config)
        Logging::logger.debug 'Deleting logs from previous EMR execution...'

        # Getting and filtering non-specified buckets
        buckets = [
          config[:s3][:buckets][:enriched][:good],
          config[:s3][:buckets][:shredded][:good]
        ].delete_if { |x| x.to_s.empty? }
        ...
        buckets.each do |bucket|
          bucket_location = Sluice::Storage::S3::Location.new(bucket)
          files = Sluice::Storage::S3::list_files(s3, bucket_location)

          unless files.empty?
            Sluice::Storage::S3::delete_files(s3, files)
          end
        end
