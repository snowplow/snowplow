  ...
  module EmrEtlRunner
    class Runner
      ...
      def run
        ...
        skips = @args[:skip]
        steps = {
          :staging => (resume.nil? or not skips.include?('staging')),
          :enrich => (resume.nil? or resume == 'enrich' or not skips.include?('enrich')),
          :shred => (resume.nil? or [ 'enrich', 'shred' ].include?(resume) or
            not skips.include?('shred')),
          :es => (resume.nil? or [ 'enrich', 'shred', 'elasticsearch' ].include?(resume) or
            not skips.include?('elastcisearch')),
          :archive_raw => (resume.nil? or [ 'enrich', 'shred', 'elasticsearch', 'archive_raw' ].include?(resume) or
            not skips.include?('archive_raw')),
          :rdb_load => (resume.nil? or [ 'enrich', 'shred', 'elasticsearch', 'archive_raw', 'rdb_load' ].include?(resume) or
            not skips.include?('rdb_load')),
          :analyze => (resume.nil? or [ 'enrich', 'shred', 'elasticsearch', 'archive_raw', 'rdb_load', 'analyze' ].include?(resume) or
