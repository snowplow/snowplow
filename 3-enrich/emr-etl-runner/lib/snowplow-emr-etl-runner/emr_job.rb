...
module Snowplow
  module EmrEtlRunner
    class EmrJob
      ...
      def initialize(debug, staging, enrich, staging_stream_enrich, shred, es, archive_raw, rdb_load, archive_enriched, archive_shredded, config, enrichments_array, resolver, targets, rdbloader_steps, use_persistent_jobflow, persistent_jobflow_duration)
        ...
        if shred
          ...
          storage_target_shred_args = get_rdb_shredder_target(targets[:ENRICHED_EVENTS], shred)
    ...
    private
      ...
      Contract ArrayOf[Iglu::SelfDescribingJson], String => Hash
      def self.get_rdb_shredder_target(targets, shred)
