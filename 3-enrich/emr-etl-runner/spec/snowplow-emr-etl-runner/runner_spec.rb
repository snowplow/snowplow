...
describe Runner do
  ...
  it 'should reject bogus skip stages' do
    ...
    args[:skip] = ["lunch"]
    expect {Runner.new args, config, enrichments, resolver, targets}.to raise_exception(ConfigError, "Invalid option: skip can be 'staging', 'emr', 'enrich', 'shred', 'elasticsearch', 'archive_raw', or 'analyze' not 'lunch'")
    puts targets.empty?
