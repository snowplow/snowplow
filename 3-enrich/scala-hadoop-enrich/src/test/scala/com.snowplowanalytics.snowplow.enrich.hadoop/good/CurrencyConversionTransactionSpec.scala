 ...
 */
class CurrencyConversionTransactionSpec extends Specification {
  ...
  if (sys.env.get("OER_KEY").isDefined) {
    ...
    "A job which processes a CloudFront file containing 1 valid transaction" should {
      EtlJobSpec("cloudfront", "4", true, List("geo", "netspeed"), true).
        source(MultipleTextLineFiles("inputFolder"), CurrencyConversionTransactionSpec.lines).
        sink[TupleEntry](Tsv("outputFolder")){ buf : Buffer[TupleEntry] =>
          "correctly output 1 transaction" in {
            buf.size must_== 1
            val actual = buf.head
            for (idx <- CurrencyConversionTransactionSpec.expected.indices) {
              actual.getString(idx) must beFieldEqualTo(CurrencyConversionTransactionSpec.expected(idx), withIndex = idx)
            }
          }
        }.
