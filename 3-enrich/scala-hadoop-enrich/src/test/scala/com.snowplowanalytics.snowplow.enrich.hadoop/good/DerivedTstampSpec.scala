 ...
 */
class DerivedTstampSpec extends Specification {
  ...
  "A job which processes a CloudFront file containing 1 valid page ping with campaign attribution fields" should {
    EtlJobSpec("cloudfront", "1", false, List("geo"), false, true).
      source(MultipleTextLineFiles("inputFolder"), DerivedTstampSpec.lines).
      sink[TupleEntry](Tsv("outputFolder")){ buf : Buffer[TupleEntry] =>
        "correctly output 1 page ping" in {
          buf.size must_== 1
          val actual = buf.head
          for (idx <- DerivedTstampSpec.expected.indices) {
            actual.getString(idx) must beFieldEqualTo(DerivedTstampSpec.expected(idx), withIndex = idx)
          }
        }
      }.
