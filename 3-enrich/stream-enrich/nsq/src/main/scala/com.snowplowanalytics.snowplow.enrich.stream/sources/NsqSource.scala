  ...
  def create(
    ...
    for {
      ...
      goodProducer <- NsqSink
        .validateAndCreateProducer(nsqConfig, config.out.enriched)
        .validation
      piiProducer <- (emitPii(enrichmentRegistry), config.out.pii) match {
        case (true, Some(piiStreamName)) =>
          NsqSink.validateAndCreateProducer(nsqConfig, piiStreamName).validation.map(Some(_))
      ...
      badProducer <- NsqSink
        .validateAndCreateProducer(nsqConfig, config.out.bad)
        .validation
    } yield
      new NsqSource(
        goodProducer,
