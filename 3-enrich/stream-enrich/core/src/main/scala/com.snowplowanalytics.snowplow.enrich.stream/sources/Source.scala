...
object Source {
  ...
  def adjustOversizedFailureJson(
    value: SelfDescribingData[BadRow],
    maxSizeBytes: Int,
    processor: Processor
  ): SelfDescribingData[BadRow] = {
    val json = SelfDescribingData[Json](value.schema, value.data.asJson).asJson.noSpaces
    val size = getSizeBr(value)
    SelfDescribingData[BadRow](
      oversizedBadRow,
      BadRow(
  ...
  val getSizeBr: SelfDescribingData[BadRow] => Int =
...
abstract class Source(
  ...
  ): List[ValidatedNel[(SelfDescribingData[BadRow], String), (String, String, Option[String])]] = {
    val canonicalInput: ValidatedNel[SelfDescribingData[BadRow], Option[CollectorPayload]] =
      ThriftLoader.toCollectorPayload(binaryData, processor)
    val processedEvents: List[ValidatedNel[SelfDescribingData[BadRow], EnrichedEvent]] =
