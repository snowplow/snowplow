...
object GooglePubSubEnrich extends Enrich {
  ...
  override def getSource(
    ...
    GooglePubSubSource
      .createAndInitialize(streamsConfig, resolver, enrichmentRegistry, tracker)
   ...
   */
  private def lookupDatastoreResolver(kind: String, keyName: String): \/[String, String] =
    for {
      ...
      valueOrNull <- utils
        .toEither(Try(datastore.get(key, List.empty[ReadOption]: _*)))
        .leftMap(_.getMessage)
    ...
    val jsons: Validation[String, List[String]] = enrichmentArg
      .map {
        ...
        case DatastoreRegex(kind, keyNamePrefix) =>
          lookupDatastoreEnrichments(kind, keyNamePrefix).validation
            .leftMap(_.getMessage)
            .flatMap {
  ...
  private def lookupDatastoreEnrichments(
    kind: String,
    keyNamePrefix: String): \/[Throwable, List[String]] =
    for {
      datastore <- DatastoreOptions.getDefaultInstance().getService().right
      query = Query.newEntityQueryBuilder().setKind(kind).build()
      // weird varargs overloading causes conflicts
      values <- utils
        .toEither(Try(datastore.run(query, List.empty[ReadOption]: _*)))
