      ...
      val config = getStagingCredentials.map { case (accessKeyId, secretAccessKey) =>
        try {   // Send request to delete previously created table and wait unit it is deleted
          ...
          val client = AmazonDynamoDBClientBuilder
            .standard()
            .withRegion(SpecHelpers.dynamodbDuplicateStorageRegion)
            .withCredentials(new AWSStaticCredentialsProvider(credentials))
            .build()
          client.deleteTable(SpecHelpers.dynamodbDuplicateStorageTable)
          new Table(client, SpecHelpers.dynamodbDuplicateStorageTable).waitForDelete()
        } catch {
        ...
        DuplicateStorage.DynamoDbConfig(
          ...
          awsRegion = SpecHelpers.dynamodbDuplicateStorageRegion,
          dynamodbTable = SpecHelpers.dynamodbDuplicateStorageTable
        )
      }

      config.flatMap(DuplicateStorage.initStorage).fold(e => throw new RuntimeException(e.toString), s => s)
    }
    ...
    private[good] def getStoredItems: List[String] = {
      getStagingCredentials.map { case (accessKeyId, secretAccessKey) =>
        ...
        val client = AmazonDynamoDBClientBuilder
          ...
          .withCredentials(new AWSStaticCredentialsProvider(credentials))
          .build()
        val res = new ScanRequest().withTableName(SpecHelpers.dynamodbDuplicateStorageTable)
...
class CrossBatchDeduplicationSpec extends Specification {
  ...
  "A job which is provided with a two events with same event_id" should {
    ...
    val Sinks =
      JobSpecHelpers.runJobInTool(CrossBatchDeduplicationSpec.lines, true)
