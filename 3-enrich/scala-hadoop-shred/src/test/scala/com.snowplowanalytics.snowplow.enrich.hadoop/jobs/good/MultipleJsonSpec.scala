...
class MultipleJsonSpec extends Specification {
  ...
  "A job which is provided with a valid Snowplow link_click event plus website_page context" should {
    ...
    "shred the Snowplow link_click event into its appropriate path" in {
      ...
      val linkClickEventSource = ScalaSource.fromFile(new File(Sinks.output, MultipleJsonSpec.expectedEvent.path))
      linkClickEventSource.getLines.toList mustEqual Seq(MultipleJsonSpec.expectedEvent.contents)
    }
    ...
    "shred the website page_context into its appropriate path" in {
      ...
      // Scala
      import scala.io.{Source => ScalaSource}
      val pageContextSource = ScalaSource.fromFile(new File(Sinks.output, MultipleJsonSpec.expectedContext.path))
      pageContextSource.getLines.toList mustEqual Seq(MultipleJsonSpec.expectedContext.contents)
    }
