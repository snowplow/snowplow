...
class WebsitePageContextSpec extends Specification {
  ...
  "A job which is provided with a valid website page_context" should {
    ...
    "shred the website page_context into its appropriate path" in {
      val lines = JobSpecHelpers.readFile(Sinks.output, WebsitePageContextSpec.expected.path)
      lines mustEqual Seq(WebsitePageContextSpec.expected.contents)
    }

    "not shred any unexpected JSONs" in {
      val expectedFiles = List("atomic-events", WebsitePageContextSpec.expected.path)
