 ...
 */
class ForwardCompatibleContextSpec extends Specification {

  "A job which is provided with a valid website page_context" should {

    val Sinks =
      JobSpecHelpers.runJobInTool(ForwardCompatibleContextSpec.lines)
    ...
    "shred the website page_context into its appropriate path" in {
      ...
      val pageContextSource = ScalaSource.fromFile(new File(Sinks.output, ForwardCompatibleContextSpec.expected.path))
      pageContextSource.getLines.toList mustEqual Seq(ForwardCompatibleContextSpec.expected.contents)
    }
