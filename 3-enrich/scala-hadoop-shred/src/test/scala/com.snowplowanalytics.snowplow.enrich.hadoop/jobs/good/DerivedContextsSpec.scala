...
class DerivedContextsSpec extends Specification {
  ...
  "A job which is provided with a valid website page_context" should {
    ...
    "shred the website page_context into its appropriate path" in {
      ...
      val pageContextSource = ScalaSource.fromFile(new File(Sinks.output, DerivedContextsSpec.expected.path))
      pageContextSource.getLines.toList mustEqual Seq(DerivedContextsSpec.expected.contents)
    }
