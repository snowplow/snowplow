...
object MissingJsonSchemaPessimisticSpec {
  ...
  val expected = (line: String) => s"""{"line":"${line}","errors":[{"level":"error","message":"Cannot load schema iglu://com.snowplowanalytics.website/fake_context/jsonschema/1-0-0 from Iglu"}]}""".format(line)
}
