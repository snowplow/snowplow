...
object SpecHelpers {
  ...
  def getStagingCredentials = {
    val configValidation = getEnv("AWS_STAGING_ACCESS_KEY_ID") |@| getEnv("AWS_STAGING_SECRET_ACCESS_KEY")
    configValidation { (accessKeyId, secretAccessKey) => (accessKeyId, secretAccessKey) }
  }
}
