...
/**
 * Check that all tuples in a page view in the CloudFront format changed in April 2014
 * are successfully extracted.
 * For details: https://forums.aws.amazon.com/thread.jspa?threadID=134017&tstart=0#
 */
class FutureCfLineSpec extends Specification with EnrichJobSpec {
  import EnrichJobSpec._
  override def appName = "future-cf-line"
  sequential
  "A job which processes a CloudFront file containing 1 valid page view" should {
    runEnrichJob(FutureCfLineSpec.lines, "cloudfront", "1", false, List("geo"))
