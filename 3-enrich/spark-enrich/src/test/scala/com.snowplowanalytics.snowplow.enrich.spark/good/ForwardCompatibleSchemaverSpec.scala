 ...
 */
class ForwardCompatibleSchemaverSpec extends Specification {
  ...
  "A job which processes a Clojure-Tomcat file containing a POST raw event representing 3 events" should {
    EtlJobSpec("clj-tomcat", "2", true, List("geo")).
      source(MultipleTextLineFiles("inputFolder"), ForwardCompatibleSchemaverSpec.lines).
      sink[TupleEntry](Tsv("outputFolder")){ buf : Buffer[TupleEntry] =>
        "correctly output 1 page view and 2 structured events" in {
          buf.size must_== 3
          for (ei <- ForwardCompatibleSchemaverSpec.expected.indices; fi <- ForwardCompatibleSchemaverSpec.expected(ei).indices) {
            buf(ei).getString(fi) must beFieldEqualTo(ForwardCompatibleSchemaverSpec.expected(ei)(fi), withIndex = fi)
          }
        }
      }.
