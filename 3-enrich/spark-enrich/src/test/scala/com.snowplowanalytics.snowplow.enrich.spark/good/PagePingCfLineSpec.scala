...
/** Check that all tuples in a page ping (CloudFront format) are successfully extracted. */
class PagePingCfLineSpec extends Specification with EnrichJobSpec {
  import EnrichJobSpec._
  override def appName = "page-ping-cf-lines"
  sequential
  "A job which processes a CloudFront file containing 1 valid page ping" should {
    runEnrichJob(PagePingCfLineSpec.lines, "cloudfront", "1", false, List("geo"))
    ...
    "correctly output 1 page ping" in {
      ...
      for (idx <- PagePingCfLineSpec.expected.indices) {
        actual(idx) must beFieldEqualTo(PagePingCfLineSpec.expected(idx), idx)
      }
    }
