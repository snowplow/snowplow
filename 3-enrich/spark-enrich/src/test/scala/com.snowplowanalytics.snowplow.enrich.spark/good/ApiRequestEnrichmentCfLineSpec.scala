...
/**
 * Holds the input and expected data for the test
 */
 ...
 */
class ApiRequestEnrichmentCfLineSpec extends Specification {
  ...
  if (ApiRequestEnrichmentCfLineSpec.continuousIntegration) {
    ...
    "A job which processes a CloudFront file containing 1 valid custom unstructured event, derived context and custom contexts" should {
      EtlJobSpec("cloudfront", "1", false, List("geo", "domain"), apiRequest = true).
        source(MultipleTextLineFiles("inputFolder"), ApiRequestEnrichmentCfLineSpec.lines).
        sink[TupleEntry](Tsv("outputFolder")){ buf : Buffer[TupleEntry] =>
          "correctly attach derived context fetched from REST server" in {
            ...
            for (idx <- ApiRequestEnrichmentCfLineSpec.expected.indices) {
              actual.getString(idx) must beFieldEqualTo(ApiRequestEnrichmentCfLineSpec.expected(idx), withIndex = idx)
            }
          }
        }.
    ...
    println("WARNING: Skipping APIRequestEnrichmentCfLineSpec as no CI=true environment variable was found and integration-tests/api-lookup-test.py probably not running")
  }
}
