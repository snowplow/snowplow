...
class DelimiterTest extends Specification {
  ...
  // Row without a querystring on the end Not sure what is wrong with this row, but it's erroring
  ...
  "A SnowPlow row where fields contain a Hive delimiters (newline)" should {

    val actual = SnowPlowEventDeserializer.deserializeLine(badRow, DEBUG).asInstanceOf[SnowPlowEventStruct]
  
    "strip the Hive delimiters (newline)" in {
