...
object DuplicateStorage {
  ...
  /**
    * Set event ttl to 180 days
    */
  val LifeSpan = 2592000 * 6
  ...
  /**
    ...
    */
  class DynamoDbStorage private[eventpopulator](client: AmazonDynamoDB, table: String) extends DuplicateStorage {
    import DynamoDbStorage._

    def put(eventId: String, eventFingerprint: String, etlTstamp: String): Boolean = {
      val etl: Int = (DateTime.parse(etlTstamp, RedshiftTstampFormat).getMillis / 1000).toInt
      val ttl = etl + LifeSpan
  ...
  object DynamoDbStorage {
    ...
    /**
      ...
      */
    private[eventpopulator] def createTable(client: AmazonDynamoDB, name: String): TableDescription = {
      val pks = List(
    ...
    /**
      ...
      */
    private[eventpopulator] def attributeValues(attributes: Seq[(String, Any)]): java.util.Map[String, AttributeValue] =
