...
object Main {
  ...
  private val parser = new scopt.OptionParser[RawJobConf](generated.ProjectMetadata.name) {
    ...
    opt[String]('i', "enriched-archive").required()
      ...
      .validate(s => if(s.startsWith(S3Prefix)) success else failure(s"--enrichedIn S3 path should start with $S3Prefix"))
  ...
  private def normalizeBucket(bucket: String): String = {
    val schemaless = bucket.stripPrefix(S3Prefix) // drop s3:// prefix
