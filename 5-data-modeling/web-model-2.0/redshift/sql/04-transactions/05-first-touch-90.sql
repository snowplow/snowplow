...
CREATE TABLE scratch.web_first_marketing_touch_90_days
...
AS (
  ...
  WITH step1 AS (
    ...
    SELECT
      ...
      FIRST_VALUE(marketing_event_id IGNORE NULLS) OVER (PARTITION BY domain_userid ORDER BY marketing_event_timestamp, transaction_event_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS first_marketing_event_90_id,
      transaction_event_id,
      event_type

    FROM step1

    WHERE associated_mkt_event_tstamp >= date_add('day', -90, nvl(transaction_event_timestamp, marketing_event_timestamp)) -- only marketing touches from the last 90 days before the transaction took place
  ...
  SELECT
    ...
    s3.first_marketing_event_90_id,
  ...
  RIGHT JOIN scratch.web_marketing_touches AS m
    ON s3.first_marketing_event_90_id = m.event_id -- only perform the join for the first touch event
