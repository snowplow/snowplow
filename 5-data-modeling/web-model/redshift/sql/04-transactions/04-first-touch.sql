...
CREATE TABLE scratch.web_first_marketing_touch
...
AS (
  ...
  WITH step1 AS (
    ...
    LEFT JOIN scratch.web_marketing_touches_and_transaction_events AS b -- cartesian join is on purpose
    ...
    SELECT
      ...
      FIRST_VALUE(marketing_event_id IGNORE NULLS) OVER (PARTITION BY domain_userid ORDER BY marketing_event_timestamp, transaction_event_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS first_marketing_event_id,
  ...
  RIGHT JOIN scratch.web_marketing_touches AS m
    ON s3.first_marketing_event_id = m.event_id -- only perform the join for the first touch event
