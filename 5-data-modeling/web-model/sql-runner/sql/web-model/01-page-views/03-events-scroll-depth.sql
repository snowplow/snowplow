...
AS (
  ...
  WITH prep AS (
    ...
    SELECT
      ...
      LEAST(GREATEST(MIN(NVL(ev.pp_yoffset_min, 0)), 0), MAX(ev.br_viewheight)) AS vmin, -- should be zero (edge case: not zero because the pv event is missing - but these are not in {{.scratch_schema}}.web_page_context so not an issue)
