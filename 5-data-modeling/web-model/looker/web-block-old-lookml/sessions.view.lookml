...
- view: sessions
  ...
  - dimension: first_or_returning_session
    type: string
    sql_case:
      First session: ${session_index} = 1
      Returning session: ${session_index} > 1
      else: Error
    group_label: 'Session'
    hidden: true
  ...
  - dimension: session_start_window
    sql_case:
      current_period: ${session_start_time} >= DATEADD(day, -28, GETDATE())
      previous_period: ${session_start_time} >= DATEADD(day, -56, GETDATE()) AND ${session_start_time} < DATEADD(day, -28, GETDATE())
      else: unknown
    hidden: true
  ...
  - dimension: geo_latitude
    ...
    group_label: 'Location'
    hidden: true # use geo_location instead
  ...
  - dimension: geo_timezone
    ...
    group_label: 'Location'
    hidden: true # use os_timezone instead
